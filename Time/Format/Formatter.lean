...
namespace Time.Format.Formatter

open Time.Date
open Time.Time
open Lean.Parsec

inductive DateTimeFormat
  | YYYY
  | YY
  | MMMM
  | MMM
...
def isNonLetter (c: Char) : Bool := ¬isLetter c

def specParserToken : Lean.Parsec DateTimeFormat
...
def specParser : Lean.Parsec Format :=
  (Array.toList <$> Lean.Parsec.many specParserToken) <* eof

def specParse (s: String) : Except String Format :=
  specParser.run s
...
def unabbrevMonth (month: Month) : String :=
...
def abbrevMonth (month: Month) : String :=
  match month.val, month.bounds with
  | 1, _ => "Jan"
  | 2, _ => "Feb"
  | 3, _ => "Mar"
...
def abbrevDayOfWeek (day: Weekday) : String :=
  match day with
  | .sun => "Sun"
  | .mon => "Mon"
  | .tue => "Tue"
...
def formatWithDate (date : DateTime tz) : DateTimeFormat → String
...
@[simp]
def SingleFormatType : DateTimeFormat → Type
  | .YYYY => Nat
  | .YY => Nat
  | .MMMM => Month
  | .MMM => Month
...
def FormatType (result: Type) : Format → Type
...
def parseMonth : Lean.Parsec Month
...
def parseMonthUnabbrev : Lean.Parsec Month
...
def parseWeekday : Lean.Parsec Weekday
...
def parseWeekdayUnnabrev : Lean.Parsec Weekday
...
def parserUpperHourMarker : Lean.Parsec HourMarker
  :=  (pstring "AM" *> pure HourMarker.am)
  <|> (pstring "PM" *> pure HourMarker.pm)

def parserLowerHourMarker : Lean.Parsec HourMarker
  :=  (pstring "am" *> pure HourMarker.am)
  <|> (pstring "pm" *> pure HourMarker.pm)

def parseYearTwo : Lean.Parsec Nat :=do
  let year ← twoDigit
  return if year < 70 then 2000 + year else 1900 + year

def timeOffset (colon: Bool) : Lean.Parsec Offset := do
...
def timeOrUTC (utcString: String) (colon: Bool) : Lean.Parsec Offset :=
  (pstring utcString *> pure Offset.zero) <|> timeOffset colon

def parserWithFormat : (typ: DateTimeFormat) → Lean.Parsec (SingleFormatType typ)
  | .YYYY => fourDigit
  | .YY => parseYearTwo
  ...
  | .d => transform Nat.Bounded.ofNat? (Lean.Parsec.orElse Formatter.twoDigit (λ_ => pchar ' ' *> (singleDigit)))
...
def addDataInDateTime (data: Time.DateTime tz) (typ: DateTimeFormat) (value: SingleFormatType typ) : Time.DateTime tz :=
  match typ with
...
def parserFormatD (format: Format) (func: FormatType α format) : Lean.Parsec α :=
  match format with
  | .cons entry xs => do
...
def parserFormat (format: Format) (func: DateTime tz) : Lean.Parsec (DateTime tz) :=
  match format with
  | .cons entry xs => do
    let data ← parserWithFormat entry
...
def parserToDate (format: Format) : Lean.Parsec (DateTime tz) :=
...
def Formats.RFC3339 : Format := Format.spec! "YYYY-MM-DD'T'hh:mm:ssZZZZZ"

def Formats.ISO8601 : Format := Format.spec! "YYYY-MM-DD'T'hh:mm:ss'Z'"

def Formats.AmericanDate : Format := Format.spec! "MM/DD/YYYY"

def Formats.EuropeanDate : Format := Format.spec! "DD/MM/YYYY"

def Formats.Time12Hour : Format := Format.spec! "HH:mm:ss aa"

def Formats.Time24Hour : Format := Format.spec! "hh:mm:ss"

def Formats.SQLDate : Format := Format.spec! "YYYY-MM-DD"

def Formats.DayMonthYear : Format := Format.spec! "EEEE, MMMM D, YYYY"

def Formats.LongDateFormat : Format := Format.spec! "EEEE, MMMM D, YYYY hh:mm:ss"

def Formats.AscTime : Format := Format.spec! "EEE MMM d hh:mm:ss YYYY"

def Formats.RFC822 : Format := Format.spec! "DD MMM YYYY hh:mm ZZZ"

def Formats.RFC850 : Format := Format.spec! "EEEE, DD-MMM-YY hh:mm:ss ZZZ"
