...
/-- Get the current Epoch time by fetching the current number of seconds since the Unix epoch. -/
def Epoch.now : IO Epoch := do
  let secs ← Time.primGetCurrentEpochSeconds
  pure ⟨secs % 253402300800, Nat.mod_lt secs (by decide)⟩

/-- Calculate the difference between two Epoch times in seconds. -/
def Epoch.diff (e1 e2: Epoch) : Int :=
  e1.val - e2.val

/-- Add a given number of seconds to an Epoch time. -/
